...
int exec_builtin(int argc, char**argv) {
    ...
    if (strcmp(argv[0], "cd") == 0) 
    {
        ...
        if(chdir(argv[1]) == -1)    //更改当前工作目录。 参数：Path 目标目录，可以是绝对目录或相对目录。 返回值：成功返回0 ，失败返回-1
        {
...
int main() {
    ...
    while (1) {
        ...
        printf("shell: %s -> ", current_working_directory);
        fflush(stdout);
